...
<form method="POST" action="{{ url('posts') }}">
    ...
    <div class="form-control">
        ...
        <input type="text" id="txttitle" name="txttitle" placeholder="title" value="{{ old('txttitle') }}"
    ...
    <div class="form-control">
        ...
        <select class="select select-bordered @error('txtcategory') is-invalid @enderror" id="txtcategory"
            ...
            @foreach ($category as $option)
                <option value="{{ $option->id }}" {{ old('txtcategory') == $option->id ? 'selected' : '' }}>
                    {{ $option->name }}</option>
            @endforeach
        </select>
        @error('txtcategory')
    ...
    <div class="form-control">
        ...
        <select class="select select-bordered @error('txtuser') is-invalid @enderror" id="txtuser" name="txtuser">
            ...
            @foreach ($user as $option)
                <option value="{{ $option->id }}" {{ old('txtuser') == $option->id ? 'selected' : '' }}>
                    {{ $option->name }}</option>
            @endforeach
        </select>
        @error('txtuser')
    ...
    <div class="form-control">
        ...
        <input type="text" id="txtslug" name="txtslug" placeholder="title" value="{{ old('txtslug') }}"
    ...
    <div class="form-control">
        ...
        <textarea class="textarea textarea-bordered h-24 @error('txtcontent') is-invalid @enderror" placeholder="content"
            id="txtcontent" name="txtcontent">{{ old('txttitle') }}</textarea>
        @error('txtcontent')
            <div class="alert alert-danger text-error"><i class="fa-solid fa-circle-exclamation"></i>{{ $message }}
            </div>
        @enderror
